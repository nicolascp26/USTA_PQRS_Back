export const SQL_MENSAJES = {
  ...
  OBTENER_HILO_MENSAJES: "WITH RECURSIVE hilo_mensajes AS (\
    SELECT mensaje_id, mensaje_codpadre, mensaje_id_usuario, mensaje_estado, mensaje_titulo, mensaje_detalle, mensaje_fecha,mensaje_usuario_asignado\
    FROM mensaje m INNER JOIN usuario u ON u.usuario_id = m.mensaje_id_usuario\
    WHERE mensaje_id = $1\
    UNION ALL\
      SELECT msj.mensaje_id, msj.mensaje_codpadre, msj.mensaje_id_usuario, msj.mensaje_estado, msj.mensaje_titulo, msj.mensaje_detalle, msj.mensaje_fecha,msj.mensaje_usuario_asignado\
      FROM mensaje msj\
    	JOIN hilo_mensajes ON msj.mensaje_codpadre = hilo_mensajes.mensaje_id)\
    SELECT hm.mensaje_id, hm.mensaje_estado, hm.mensaje_titulo, hm.mensaje_detalle, hm.mensaje_fecha, u.usuario_nombres,u.usuario_apellidos,hm.mensaje_id_usuario,hm.mensaje_usuario_asignado\
  ...
  REABRIR_SOLICITUD: "UPDATE mensaje \
    SET mensaje_estado = 3 \
    WHERE mensaje_id = $1;",
  REASIGNAR_SOLICITUD: "UPDATE mensaje \
    SET mensaje_usuario_asignado = $2 \
    WHERE mensaje_id = $1;"
}
